/**
 * AI explanation layer (backend).
 *
 * Design rule: the analytics engine produces every number; this layer only
 * turns those numbers into language. The default provider is a free, offline
 * rule-based explainer so the core product needs no paid API and no key.
 *
 * To plug in a hosted model later, implement `AiProvider` here and read the
 * key from the server environment (AI_API_KEY). Never let the model invent
 * figures — pass it the computed metrics as context and ask only for
 * explanation.
 */
...
export interface AiProvider {
  name: string;
  explainPortfolio(metrics: PortfolioMetricsLike): AiInsight[];
}

/** Structural subset of the analytics engine's metrics output. */
export interface PortfolioMetricsLike {
  totalValue: number;
  ...
  sectorAllocation: Array<{ sector: string; value: number; pct: number }>;
  holdingCount: number;
  /** False when history is too short for portfolio-level risk statistics. */
...
export function explainPortfolio(metrics: PortfolioMetricsLike): AiInsight[] {
  return offlineExplainer.explainPortfolio(metrics);
}

export function explainBuySimulation(
  symbol: string,
  amount: number,
  result: {
    classification: string;
    fitScore: number;
    before: PortfolioMetricsLike;
    after: PortfolioMetricsLike;
    reasons: string[];
  },
): AiInsight {
  const dir =
...
export function explainSellSimulation(
  symbol: string,
  pct: number,
  result: { recommendation: string; proceeds: number; reasons: string[] },
): AiInsight {
  return {
    title: `Signal: ${result.recommendation}`,
    body: `Simulating a ${pct}% sale of ${symbol} releases about ${result.proceeds.toFixed(0)} and changes the portfolio profile as computed. ${result.reasons.join(" ")} This is a simulation only — no transaction is recorded, and current indicators describe risk rather than certainty about future prices.`,
